...
export const addServicesToVisit = async (
  ...
  const visitId =
    typeof visitIdOrParams === "number"
      ? visitIdOrParams
      : visitIdOrParams.visitId;
  const service_ids = Array.isArray(maybeServiceIds)
    ? maybeServiceIds
    : (visitIdOrParams as { visitId: number; service_ids: number[] })
        .service_ids;
...
export const updateRequestedServiceDetails = async (
  ...
  const response = await apiClient.put<{ data: RequestedService }>(
    `/requested-services/${rsId}`,
    data,
  ); // Or `/visits/${visitId}/requested-services/${rsId}`
  return response.data.data;
};

export const getRequestedServicesForVisit = async (
  visitId: number,
): Promise<RequestedService[]> => {
  // Assuming backend returns { data: RequestedService[] }
  const response = await apiClient.get<{ data: RequestedService[] }>(
    `${VISIT_SERVICES_API_BASE}/${visitId}/requested-services`,
  );
  return response.data.data;
};

export const removeRequestedServiceFromVisit = async (
  visitId: number,
  requestedServiceId: number,
): Promise<void> => {
  await apiClient.delete(
    `${VISIT_SERVICES_API_BASE}/${visitId}/requested-services/${requestedServiceId}`,
  );
};
...
export const updateDoctorVisitStatus = async (
  ...
  const response = await apiClient.put<{ data: DoctorVisit }>(
    `${VISIT_API_URL_BASE}/${visitId}/status`,
    { status },
  );
  return response.data.data;
};
...
export const reassignDoctorVisitToShift = async (
  ...
  const response = await apiClient.post<{ data: DoctorVisit }>(
    `/doctor-visits/${visitId}/reassign-shift`,
    { target_doctor_shift_id: targetDoctorShiftId },
  );
  return response.data.data;
};
