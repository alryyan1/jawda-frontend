...
import {
  getAdmissionLedger,
  getAdmissionById,
  deleteAdmissionTransaction,
  exportAdmissionLedgerPdf,
...
export default function AdmissionLedgerTab({
  admissionId,
}: AdmissionLedgerTabProps) {
  const queryClient = useQueryClient();
  const [depositDialogOpen, setDepositDialogOpen] = useState(false);
  const [discountDialogOpen, setDiscountDialogOpen] = useState(false);
  const [chargeDialogOpen, setChargeDialogOpen] = useState(false);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [daysCalculationDialogOpen, setDaysCalculationDialogOpen] =
    useState(false);
  ...
  const deleteTransactionMutation = useMutation({
    mutationFn: (transactionId: number | string) =>
      deleteAdmissionTransaction(admissionId, Number(transactionId)),
    onSuccess: () => {
  ...
  const getCalculationPeriod = () => {
    if (!admissionDate) return "غير محدد";
  ...
  const getCalculationExplanation = () => {
    if (!admissionDate) return "لم يتم تحديد وقت الدخول";
    const date = new Date(admissionDate);
    if (isNaN(date.getTime())) return "غير محدد";
    const hour = date.getHours();
    if (hour >= 7 && hour < 12) {
  ...
  return (
    ...
    >
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                ...
                <Box sx={{ flex: 1 }}>
                  ...
                  >
                    {admissionDate
                      ? new Date(admissionDate).toLocaleString("ar-EG")
                      : "-"}
                  </Typography>
                </Box>
              </Box>
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                ...
                <Box sx={{ flex: 1 }}>
                  ...
                  >
                    {dischargeDate
                      ? new Date(dischargeDate).toLocaleString("ar-EG")
                      : "-"}
                  </Typography>
                </Box>
              </Box>
        ...
        >
          ...
          >
            <CardContent sx={{ py: 1.5, px: 2 }}>
              ...
              <Typography
                variant="h5"
                sx={{ fontWeight: 700, color: "#DC2626", fontSize: "1.5rem" }}
              >
                {formatNumber(summary.total_debits)}
              </Typography>
              <Typography
                variant="caption"
                sx={{ color: "#991B1B", fontSize: "0.65rem" }}
              >
                الرسوم والخدمات
              </Typography>
            </CardContent>
          ...
          >
            <CardContent sx={{ py: 1.5, px: 2 }}>
              ...
              <Typography
                variant="h5"
                sx={{ fontWeight: 700, color: "#16A34A", fontSize: "1.5rem" }}
              >
                {formatNumber(summary.total_credits)}
              </Typography>
              <Typography
                variant="caption"
                sx={{ color: "#166534", fontSize: "0.65rem" }}
              >
                المبالغ المدفوعة
              </Typography>
            </CardContent>
          ...
          >
            <CardContent sx={{ py: 1.5, px: 2 }}>
              ...
              <Typography
                variant="h5"
                sx={{ fontWeight: 700, color: "#D97706", fontSize: "1.5rem" }}
              >
                {formatNumber(summary.total_discounts || 0)}
              </Typography>
              <Typography
                variant="caption"
                sx={{ color: "#92400E", fontSize: "0.65rem" }}
              >
                الخصومات المطبقة
              </Typography>
            </CardContent>
      ...
      >
                ...
                <Card variant="outlined" sx={{ bgcolor: "action.hover" }}>
                  <CardContent sx={{ py: 1.5 }}>
                    ...
                    <Typography variant="body2" sx={{ mb: 1 }}>
                      إذا دخل المريض في أي وقت من الساعة 7:00 صباحاً وحتى 12:00
                      ظهراً، فإن يومه ينتهي في نفس التوقيت من اليوم التالي (أي
                      يُكمل 24 ساعة كاملة).
                    </Typography>
                    <Typography
                      variant="body2"
                    ...
                    >
                      مثال: دخل الساعة 9:00 صباحاً اليوم، ينتهي يومه الساعة 9:00
                      صباحاً غداً.
                    </Typography>
                  </CardContent>
                </Card>
                ...
                <Card variant="outlined" sx={{ bgcolor: "action.hover" }}>
                  <CardContent sx={{ py: 1.5 }}>
                    <Typography
                      ...
                      sx={{ mb: 0.5 }}
                    >
                      نظام اليوم الكامل (الدخول المسائي/المتأخر: 1:00 ظ - 6:00 ص
                      اليوم التالي)
                    </Typography>
                    <Typography variant="body2" sx={{ mb: 1 }}>
                      إذا دخل المريض من الساعة 1:00 ظهراً وحتى 6:00 صباحاً من
                      اليوم التالي، فإن يومه ينتهي "حكماً" عند الساعة 12:00
                      ظهراً من اليوم التالي، بغض النظر عن عدد الساعات الفعلية.
                    </Typography>
                    <Typography
                      variant="body2"
                      sx={{ fontStyle: "italic", color: "text.secondary" }}
                    >
                      مثال: دخل الساعة 4:00 عصراً أو حتى 3:00 فجراً، بمجرد وصول
                      الساعة 12:00 ظهراً يُعتبر قد أتمّ يوماً كاملاً.
                    </Typography>
                  </CardContent>
                </Card>
                ...
                <Card variant="outlined" sx={{ bgcolor: "action.hover" }}>
                  <CardContent sx={{ py: 1.5 }}>
                    ...
                    <Typography variant="body2">
                      يتم احتساب المدة بالطريقة التقليدية: عدد الأيام بين تاريخ
                      الدخول والخروج + 1 (لضمان احتساب يوم واحد على الأقل).
                    </Typography>
                  </CardContent>
                </Card>
              </Box>
        ...
        <DialogActions>
          <Button
            onClick={() => setDaysCalculationDialogOpen(false)}
            variant="contained"
          >
            فهمت
          </Button>
