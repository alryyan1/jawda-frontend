...
import { RoleFormData, Role, Permission } from '@/types/auth'; // Adjust path if types are elsewhere
...
const getRoleFormSchema = (t: Function) => z.object({
...
const RoleFormPage: React.FC<RoleFormPageProps> = ({ mode }) => {
  ...
  // Group permissions by resource for better UI (e.g., "users", "doctors")
  const groupedPermissions = allPermissions?.reduce((acc, permission) => {
    const [resource, action] = permission.name.split(' '); // Simple split, assumes "action resource" or "resource action"
    const groupName = action && action.length > 2 ? action : resource; // Or a more sophisticated grouping logic
                   ...
                   <p className="text-sm text-muted-foreground py-4">{t('roles:form.noPermissionsAvailable')}</p> : (
                    <ScrollArea className="h-auto max-h-[50vh] w-full rounded-md border p-4">
                      <div className="space-y-4">
                        {Object.entries(groupedPermissions).sort(([groupA], [groupB]) => groupA.localeCompare(groupB)).map(([groupName, perms]) => (
                          <div key={groupName}>
