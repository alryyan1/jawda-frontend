...
const CashReconciliationPage: React.FC = () => {
  const { t, i18n } = useTranslation(['cashReconciliation', 'common']);
  ...
  return (
    <div className="container mx-auto py-8">
      ...
      <div className="mb-6 max-w-sm">
        <label htmlFor="shift-select" className="text-sm font-medium text-muted-foreground">{t('selectShift')}</label>
        <Select
        ...
        >
            <SelectTrigger id="shift-select"><SelectValue placeholder={t('selectShiftPlaceholder')} /></SelectTrigger>
            <SelectContent>
                {shiftsList?.map(s => (
      ...
      {isLoading ? (
        ...
        <Card className="p-10 text-center text-muted-foreground">{t('pleaseSelectShift')}</Card>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* Right Column: Denomination Calculator */}
          <Card className="md:col-span-2">
            <CardHeader><CardTitle>{t('calculatorTitle')}</CardTitle></CardHeader>
            <CardContent>
              <div className="space-y-2">
                {/* Headers */}
                <div className="grid grid-cols-4 gap-4 px-2 font-semibold text-sm text-muted-foreground">
                  <div className="text-center">{t('denominationHeader')}</div>
                  <div className="text-center">{t('countHeader')}</div>
                  <div className="text-center">{t('sumOfCountsHeader')}</div>
                  <div className="text-center">{t('totalHeader')}</div>
                </div>
                {/* Denomination Rows */}
                {denominations.map(deno => (
                  <div key={deno.id} className="grid grid-cols-4 gap-4 items-center">
                    ...
                    <Input
                      ...
                      placeholder={t('countInputPlaceholder')}
          ...
          <div className="space-y-4">
            <Card>
              <CardHeader><CardTitle>{t('summary.employeeAccount')}</CardTitle></CardHeader>
              <CardContent className="space-y-2 text-center">
                <p className="text-xs text-muted-foreground">{t('summary.totalDenominations')}</p>
                ...
                <p className="text-xs text-muted-foreground mt-4">{t('summary.totalWithExpenses')}</p>
                <p className="text-2xl font-bold">0.0</p>
                
                <p className="text-xs text-muted-foreground mt-4">{t('summary.bank')}</p>
                <p className="text-2xl font-bold">0.0</p>
              </CardContent>
            </Card>
          </div>
