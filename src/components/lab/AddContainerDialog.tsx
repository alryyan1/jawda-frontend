...
const containerSchema = z.object({
  container_name: z.string().min(1, { message: 'اسم الوعاء مطلوب' })
                  .max(50, { message: 'اسم الوعاء يجب أن يكون أقل من 50 حرف' }),
});
...
const AddContainerDialog: React.FC<AddContainerDialogProps> = ({ onContainerAdded, triggerButton }) => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (newContainer: Container) => {
      toast.success('تم إضافة الوعاء بنجاح');
