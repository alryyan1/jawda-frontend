...
const LabActionsPane: React.FC<LabActionsPaneProps> = ({
  ...
  useEffect(() => {
    ...
    const fileRef = ref(
      storage,
      `results/${storageName}/${currentPatientData.visit_id}/result.pdf`,
    );
    getMetadata(fileRef)
      .then((metadata) => {
  ...
  const toggleLockMutation = useMutation({
    ...
    onSuccess: (updatedPatient, variables) => {
      ...
      toast.success(
        variables.lock ? "تم قفل النتائج بنجاح" : "تم إلغاء قفل النتائج بنجاح",
      );
      ...
      if (updatedPatient) {
        ...
        queryClient.setQueryData(
          ["patientDetailsForActionPane", variables.patientId],
          updatedPatient,
        );

        // Also update other patient detail caches that might exist
        queryClient.setQueryData(
          ["patientDetailsForInfoPanel", variables.patientId],
          (old: { data?: { result_is_locked?: boolean } } | undefined) => {
            if (old?.data) {
              return {
                ...old,
                data: {
        ...
        queryClient.setQueryData(
          ["patientDetailsForLabDisplay", variables.patientId],
          (old: { data?: { result_is_locked?: boolean } } | undefined) => {
            if (old?.data) {
              return {
                ...old,
        ...
        queryClient.setQueryData(
          ...
          (
            old:
              | { data?: { patient_id: number; result_is_locked?: boolean }[] }
              | undefined,
          ) => {
            if (old?.data) {
              return {
                ...old,
                data: old.data.map(
                  (item: { patient_id: number; result_is_locked?: boolean }) =>
      ...
      queryClient.invalidateQueries({
        queryKey: ["patientDetailsForActionPane", variables.patientId],
      });
      queryClient.invalidateQueries({
        queryKey: ["patientDetailsForInfoPanel", variables.patientId],
      });
      queryClient.invalidateQueries({
        queryKey: ["patientDetailsForLabDisplay", variables.patientId],
      });
  ...
  const handleToggleLock = () => {
    ...
    toggleLockMutation.mutate({
      patientId: patientIdForLock,
      lock: !currentLockStatus,
    });
  };
  ...
  const setDefaultMutation = useMutation({
    mutationFn: (labRequestId: number) =>
      setLabRequestResultsToDefault(labRequestId),
    onSuccess: (updatedLabRequest) => {
      // toast.success('تم إعادة تعيين النتائج إلى القيم الافتراضية بنجاح');
      queryClient.invalidateQueries({
        queryKey: ["labRequestForEntry", updatedLabRequest.id],
      });
      queryClient.invalidateQueries({
        queryKey: ["labRequestsForVisit", selectedVisitId],
      }); // If visitId is relevant
    ...
    onError: (error: Error) => {
      toast.error(
        (error as { response?: { data?: { message?: string } } }).response?.data
          ?.message || "فشل إعادة تعيين النتائج إلى القيم الافتراضية",
      );
    },
  });
  ...
  const populateCbcMutation = useMutation({
    mutationFn: (payload: {
      ...
      populateCbcResults(payload.labRequestId, {
        doctor_visit_id_for_sysmex: payload.doctorVisitId,
    ...
    onSuccess: (response) => {
      if (response.status && response.data) {
        ...
        queryClient.invalidateQueries({
          queryKey: ["labRequestForEntry", response.data.id],
        });
        queryClient.invalidateQueries({
          queryKey: ["labRequestsForVisit", selectedVisitId],
        });
  ...
  const handlePopulateCbc = () => {
    if (selectedLabRequest && selectedVisitId) {
      // selectedVisitId is the DoctorVisit ID
  ...
  const populateChemistryMutation = useMutation({
    mutationFn: async (payload: {
      ...
      const response = await apiClient.post(
        `/populatePatientChemistryData/${payload.doctorVisitId}`,
        {
          main_test_id: payload.mainTestId,
        },
      );
    ...
    onSuccess: (response) => {
      if (response.status) {
        ...
        queryClient.invalidateQueries({
          queryKey: ["labRequestForEntry", selectedLabRequest?.id],
        });
        queryClient.invalidateQueries({
          queryKey: ["labRequestsForVisit", selectedVisitId],
        });
    ...
    onError: (error: Error) => {
      toast.error(
        (error as { response?: { data?: { message?: string } } }).response?.data
          ?.message || "فشل ملء نتائج الكيمياء",
      );
    },
  });
  ...
  const populateHormoneMutation = useMutation({
    mutationFn: async (payload: {
      ...
      const response = await apiClient.post(
        `/populatePatientHormoneData/${payload.doctorVisitId}`,
        {
          main_test_id: payload.mainTestId,
        },
      );
    ...
    onError: (error: Error) => {
      toast.error(
        (error as { response?: { data?: { message?: string } } }).response?.data
          ?.message || "فشل ملء نتائج الهرمونات",
      );
    },
  });
  ...
  const handleSendWhatsappDirectPdfReport = async () => {
    if (!selectedVisitId || !currentPatientData?.phone) {
  ...
  const handleUploadToFirebase = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        queryClient.setQueryData(
          ["patientDetailsForActionPane", currentPatientData.patient_id],
        ...
        queryClient.setQueryData(
          ["patientDetailsForInfoPanel", currentPatientData.patient_id],
          (old: { data?: { result_url?: string } } | undefined) => {
            if (old?.data) {
              return {
                ...old,
                data: {
            ...
            return {
              data: { ...old?.data, result_url: response.data.result_url },
            };
          },
        );

        queryClient.setQueryData(
          ["patientDetailsForLabDisplay", currentPatientData.patient_id],
          (old: { data?: { result_url?: string } } | undefined) => {
            if (old?.data) {
              return {
                ...old,
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                className="w-12 h-12"
                onClick={() => {
                  window.open(LisServerUrl, "_blank");
                }}
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                size="icon"
                className="w-12 h-12"
                onClick={handleOpenWhatsappWorkAreaDialog}
              >
              ...
              >
                {isUploadingToFirebase ? (
                  ...
                  <FontAwesomeIcon
                    ...
                    style={{
                      color: fileExistsInFirebase ? "lightgreen" : "red",
                    }}
          ...
          <TooltipTrigger asChild>
            ...
            >
              {populateCbcMutation.isPending ? (
                ...
                <FontAwesomeIcon
                  icon={faBars}
                  style={{ color: currentPatientData?.has_cbc ? "red" : "" }}
                  className="h-7! w-7"
                />
              )}
            </Button>
            ...
            >
              {populateChemistryMutation.isPending ? (
                ...
                <FontAwesomeIcon
                  icon={faBolt}
                  style={{
                    color: currentPatientData?.has_chemistry ? "yellow" : "",
                  }}
                  className="h-7! w-7!"
                />
              )}
            ...
            >
              {populateHormoneMutation.isPending ? (
                ...
                <FontAwesomeIcon
                  icon={faFlask}
                  style={{
                    color: currentPatientData?.has_hormone ? "#9b59b6" : "",
                  }}
                  className="h-7! w-7!"
                />
              )}
