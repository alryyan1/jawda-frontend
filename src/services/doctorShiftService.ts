...
export const getDoctorShiftsForReview = async (
  filters: DoctorShiftReportFilters,
): Promise<PaginatedResponse<DoctorShiftReportItem>> => {
  const response = await apiClient.get<
    PaginatedResponse<DoctorShiftReportItem>
  >("/doctor-shifts", { params: filters });
  return response.data;
};

export const getDoctorShiftFinancialSummary = async (
  doctorShiftId: number,
): Promise<DoctorShiftFinancialSummary> => {
  const response = await apiClient.get<{ data: DoctorShiftFinancialSummary }>(
    `/doctor-shifts/${doctorShiftId}/financial-summary`,
  );
  return response.data.data;
};

export const getDoctorsWithShiftStatus = async (filters: {
  search?: string;
}): Promise<DoctorWithShiftStatus[]> => {
  const response = await apiClient.get<
    DoctorWithShiftStatus[] | Record<string, DoctorWithShiftStatus>
  >("/doctors-with-shift-status", { params: filters });
  if (Array.isArray(response.data)) {
    return response.data;
  }
  // If backend returns an object (associative array), convert to array values
  return Object.values(response.data);
};

export const startDoctorShift = async (
  payload: StartShiftPayload,
): Promise<DoctorShift> => {
  const response = await apiClient.post<{ data: DoctorShift }>(
    "/doctor-shifts/start",
    payload,
  );
